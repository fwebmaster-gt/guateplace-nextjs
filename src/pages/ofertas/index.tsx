...
export default function Home({
  ...
  const offerList = products.filter((p) => p.precio_especial);

  return (
    <div className="container mx-auto">
      <Seo
        title={"Ofertas Y Descuentos"}
        description={`Encuentra las mejores ofertas y descuentos en productos`}
        image={"/oferta.png"}
      />
      <Navbar />
        ...
        >
          <div>
            <div className="bg-primary h-[100px] w-[100px] min-h-[100px] min-w-[100px] max-h-[100px] max-w-[100px] border rounded-full flex items-center justify-center">
              <img className="w-[75%]" src={"/oferta.png"} alt="ofertas" />
            </div>
      ...
      {filterProducts(offerList, search).length === 0 && (
      ...
      <div className="grid grid-cols-2 gap-4 px-4 mt-5 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
        {filterProducts(offerList, search).map((p) => (
